...
from sqlalchemy.ext.asyncio import AsyncSession
from src.database import get_db
from src.utils.permission_checker import PermissionChecker # Keep this for other routes
from src.controller.org_type_controller import *
from src.schemas.org_type_schema import *
...
@router.get("/", response_model=PaginatedOrganizationTypeResponse) # <<< MODIFIED LINE: Removed dependencies
async def get_all_organization_types(
    db: AsyncSession = Depends(get_db),
    page: int = Query(1, ge=1),
    limit: int = Query(10, ge=1),
